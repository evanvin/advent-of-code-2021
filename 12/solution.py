...
class Cave:
    ...
    def __eq__(
        self, obj: object
    ) -> bool:  # We can compare wheter two caves are the same
        return self.name == obj

    def __hash__(
        self,
    ) -> int:  # Each cave returns a unique hash so it can be used in a set or dictionary key
...
def find_path(caves: dict, extra_visit=False):
    ...
    def travel(position: Cave, my_path: list = [], has_extra_visit: bool = False):
        ...
        if (position == "start") and (len(my_path) > 1):
            return
        ...
        if position.type == "small":
            ...
            if visit_count > max_visits:
                return  # Stop if we reached the maximum visits of the small cave
